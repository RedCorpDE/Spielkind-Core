...
const schema = z
  .object({
    ...
    REGIONDO_PURCHASE_HYDRATION_MAX_ATTEMPTS: z.coerce.number().int().min(1).max(10).default(5),
    REGIONDO_PURCHASE_HYDRATION_RETRY_BASE_DELAY_MS: z.coerce.number().int().min(100).max(10_000).default(500),
    REGIONDO_PURCHASE_HYDRATION_TIMEOUT_MS: z.coerce.number().int().min(1_000).max(120_000).default(15_000),
